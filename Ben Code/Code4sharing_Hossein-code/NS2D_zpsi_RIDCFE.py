...
for iTime in range(0, M-1):
    # KK1z = RHS(zkCor[0, :])
    # KK2z = RHS(zkCor[0, :] + KK1z*(dt/2))
    # KK3z = RHS(zkCor[0, :] + KK2z*(dt/2))
    # KK4z = RHS(zkCor[0, :] + KK3z*dt)
    # zkCor[0, :] = zkCor[0, :] + (dt/6)*(KK1z + 2*KK2z + 2*KK3z + KK4z)
    zkCor[0, :] = zkCor[0, :] + dt*RHS(zkCor[0, :])
...
for iCor in range(1, M-1):
    ll = iCor - 1
    for iTime in range(0, M-1):
        zkCor[iCor, :] = zkCor[iCor, :] + \
            dt*(RHSz_mat[iCor, iTime, :]-RHSz_mat[ll, iTime, :]) + \
            dt*Quad_int1(RHSz_mat[ll, :], iTime)
        RHSz_mat[iCor, iTime+1, :] = RHS(zkCor[iCor, :])
# treat the last correction loop a little different
for iTime in range(0, M-1):
    zkCor[-1, :] = zkCor[-1, :]+dt*(RHS(zkCor[-1, :])-RHSz_mat[-1, iTime, :])\
        + dt*Quad_int1(RHSz_mat[-1, :], iTime)

# ================== INITIAL PHASE (2) ==================
for iTime in range(M-1, 2*M-2):
    iStep = iTime - (M-1)
    # prediction loop
    zkCor[0, :] = zkCor[0, :] + dt*RHSz_mat[0, M-1, :]
    # correction loops
    for ll in range(iStep):
        iCor = ll + 1
        zkCor[iCor, :] = zkCor[iCor, :] + \
            dt*(RHSz_mat[iCor, -1, :]-RHSz_mat[ll, -2, :]) + \
            dt*Quad_int2(RHSz_mat[ll, :])
...
for iTime in range(2*M-2, Nt+M-1):
    ...
    zkCor[-1, :] = zkCor[-1, :]+dt*(RHS(zkCor[-1, :])-RHSz_mat[-1, -2, :])\
        + dt*Quad_int2(RHSz_mat[-1, :])
    # ~~~~~~~~~~~ Updating Stencil ~~~~~~~~~~~
