...
export run_advection_forward_euler, Butcher1_forward_euler

function error_calculate(data_correct, data_simulated, p)
    err_data = data_correct - data_simulated
    err_norm = norm(err_data, p) # Use l2 norm in this case
...
function Butcher1_forward_euler(t_end, t_res, u_init, save_plot, save_type, return_error)
    t_array = range(0.0, t_end, t_res) |> collect
    Δt = t_end/t_res
    f(t, u) = (u-2t*u^2)/(1+t)
    u_correct(t) = (1+t)/(t+1/u_init)
    ...
    if save_plot
        new_plot = plot(t_array, u_sims)
        plot!(new_plot, t_array, u_correct.(t_array),
              color=:red, ylims=(0.2, 0.8))
    ...
    return_error && error_calculate(u_correct.(t_array), u_sims, 2)
end
