...
function main()
    ...
    @showprogress "Time Stepping..." for interval in 1:N_TIME_INTERVALS
        ...
        for i in 1:N_TIME_STEPS
            ...
            # Euler method prediction step (Find new value of fourier transform of vorticity)
            vorticities_fft[:, :, i+1, 1] = vorticities_fft[:, :, i, 1] + time_step_length * vort__dt_fft
        ...
        #------------------------------------------------------------------------------------------#
        # Euler Correction Steps for Time Interval

        for k in 1:N_CORRECTIONS
            # Get list over the times in the current interval of the time derivative of vorticity
            vort_dt_fft_vec = [dw__dt_fft(
                vorticities_fft[:, :, i, k],
                velocities_x[:, :, i, k],
                velocities_y[:, :, i, k],
                interval,
            ...
            for m in 1:N_POINTS, n in 1:N_POINTS
                vec = [vort_dt_fft_vec[i][m, n] for i in 1:N_TIME_STEPS+1]
                vort_fft_int[m, n, :] = W * vec
            end

            # Initialise approximate errors array (zero value initial condition built-in)
            errors = zeros(ComplexF64, N_POINTS, N_POINTS, N_TIME_STEPS+1)
            for i in 1:N_TIME_STEPS
                G = (
                    dw__dt_fft(                
                ...
                errors[:, :, i+1] = errors[:, :, i] + time_step_length*G 
                - (vorticities_fft[:, :, i+1, k] - vorticities_fft[:, :, i, k]) 
                + vort_fft_int[:, :, i]

                # Correct previous correction value of fourier transformed vorticities
                vorticities_fft[:, :, i+1, k+1] = vorticities_fft[:, :, i+1, k] + errors[:, :, i+1] 
