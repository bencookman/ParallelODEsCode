...
k = 4           # Number of time steps in each interval for the RIDC RK2-RK2 method
l = 2           # Number of corrections
...
plot!([minimum(log_hs), maximum(log_hs)], [minimum(log_errors), minimum(log_errors)+2*(l+1)*(maximum(log_hs) - minimum(log_hs))])
display(p)
